/**
 * server.js — Complete, updated (Groq AI + Cloudinary + Google Auth + Stripe + Email + JSON DB)
 *
 * Requires environment variables:
 * CLOUDINARY_*, STRIPE_*, GOOGLE_*, MAIL_*, GROQ_API_KEY, SESSION_SECRET, DOMAIN, STRIPE_PRICE_ID, STRIPE_WEBHOOK_SECRET
 *
 * Puppeteer notes:
...
app.post('/optimize-cv', upload.none(), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // fallback: write a minimal PDF or return HTML only. We'll return HTML preview and error notice.
      // Do not throw—return the preview so the user still gets something.
...
app.post('/send-email', bodyParser.json(), async (req, res) => {
  try {
    ...
    const opt = (db.optimizations || []).find(
      (o) => o.userId === userId && (o.pdfLocal?.endsWith(pdfFilename) || o.pdfLocal === `/generated/${pdfFilename}`)
    );
    const pdfPathLocal = path.join(generatedDir, pdfFilename);
    if (!fs.existsSync(pdfPathLocal) && !opt?.pdfUrl) return res.status(404).json({ success: false, message: 'File not found' });
    ...
    const attachments = opt?.pdfUrl ? [{ filename: pdfFilename, path: opt.pdfUrl }] : [{ filename: pdfFilename, path: pdfPathLocal }];
    await transporter.sendMail({
      from: process.env.MAIL_FROM || process.env.MAIL_USER,
      to: userEmail,
      subject: 'Your Optimized CV from Job-Fit',
      text: 'Attached is the optimized CV generated by Job-Fit.',
      attachments,
    });
